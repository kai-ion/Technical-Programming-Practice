...
Because we need to keep track of where the random pointer is pointed to, we need to store the linkedlist into
a dictionary. With the dictionary, we can map the current node to a copy of its node, and when we need to find
the copied to node pointed to, we can just search for it in the dictionary
...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        ###Explanation###
        """
        we basically just create a dictionary where we map the copied node to the old old {old : copy}. 
        We first iterate through the linkedlist, create a copy of current node, and map it to our dictionary. Then
        in the second loop, we just have to grab the mapped copy, and change its two pointers to the copied node in the
        dictionary
        """

        ###PsudoCode###
        """
        #create dictionary to map old node to copy
        #initate a none node as well
        oldToCopy = {None : None}

        #first iteration to create the dictionary entires
        cur = head
        while cur
            #create clone node and map to dictionary
            clone = Node(cur.val)
            oldToCopy[cur] = clone
            cur = cur.next
        
        #2nd iteration to correct pointers
        cur = head
        while cur
            #get the clone node
            clone = oldToCopy[cur]
