...
<!-- solution:start -->
The brute force approach would be to just start a decision tree and decide to take 1 step or 2 step at each
branch and sum up all the res, but it would be at a time of O(2 ** n).
We can optimize this approach by caching repeated steps, where some branches are just sums of other branches,
reducing the time to O(n)
The solution we are using is a dp solution but without dp, 
or basically solving for the fibonacci number, where the third number is the sum of the prev 2 number,
reducing our space to O(1)
We can get a even faster solution at log time by either using the fib formular or even using some matrix
multiplication.
...
class Solution:
    def climbStairs(self, n: int) -> int:
        ...
        ###Explanation###
        """
        For the DP solution, we would basically create a dp list of 0 up till n, then set dp at 1 and 2 to 1 and 2,
        then start iterating up till n, filling each dp[i] to be the sum of the prev two nums, i -1 and i- 2.
        Then just return dp at n

        observing that the dp solution is just solving for a fibonaci nummber at n, we can implement our solution
        of just iterating in a loop up to n, where we just add the prev 2 numbers repeatedly.
        """

        ###PsudoCode - dp solution###
        """
        #Base case where n is 1, we return 1
        if n == 1
            return 1
        ...
        #initiate the 2 base case for our dp, 1 for 1 stairs, 2 for 2 stairs
        dp[1] = 1
        dp[2] = 2

        #iterate from the 3rd index to nth index, noninclusive
        for i in range(3, n+1)
            #set the ith dp to be the sum of prev 2
            dp[i] = dp[i - 1] + dp[i - 2]
        ...
        ###PsudoCode for fib ans###
        """
        #set the first and second number to be both 1s
        first, second = 1, 1

        #iterate up to nth number
        from i in range(n - 1)
            #get a tempone to store one
            #then set first to be the sum of one and two, or the third number
            #then set two to be the new first number
            tempFirst = first
            first = first + second
            second = tempFirst
